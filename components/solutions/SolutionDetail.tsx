...
const icons: Record<string, React.ReactNode> = {
    ...
    "Regulatory Health": (
        <svg xmlns="http://www.w3.org/2000/svg" width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M9 11l3 3L22 4"/><path d="M21 12v7a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11"/></svg>
    ),
    "Contract Lifecycle": (
...
const SolutionDetail = ({ solution, index }: { solution: SolutionItem; index: number }) => {
    const isEven = index % 2 === 1;
    const bgColor = isEven ? 'bg-[#F3F2ED]' : 'bg-[#FAF9F6]';

    return (
        <section id={solution.id} className={`${bgColor} p-6 md:p-12 lg:p-20`}>
            <div className="max-w-7xl mx-auto">
                <div className="flex flex-col lg:flex-row gap-12 lg:gap-20">
                    {/* Left — Number & Icon */}
                    <div className="lg:w-1/4 flex flex-row lg:flex-col items-start gap-6">
                        <div className="w-14 h-14 rounded-2xl bg-[#C5D5E8] flex items-center justify-center text-[#4A6FA5]">
                            {icons[solution.icon]}
                        </div>
                        <span className="text-5xl md:text-6xl font-semibold text-neutral-200 tracking-tight leading-none">
                            {solution.number}
                        </span>
                    </div>
                    ...
                    <div className="lg:w-3/4">
                        <h2 className="text-3xl md:text-4xl font-semibold tracking-tight text-neutral-900 mb-3 leading-tight">
                            {solution.title}
                        </h2>
                        <p className="text-[#4A6FA5] text-sm font-medium mb-6">
                            {solution.headline}
                        </p>
                        <p className="text-neutral-600 text-base leading-relaxed mb-10 max-w-3xl">
                            {solution.description}
                        </p>

                        {/* Deliverables */}
                        <div className="border-t border-neutral-300/60 pt-8">
